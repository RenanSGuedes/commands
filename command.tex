...
Vamos imaginar que a função que descreve o traçado da circunferência depende do tempo (t), assim quando $t=0$ a curva não começou a ser traçada, mas quando $t=\pi$, metade dela já foi renderizada. Tendo esse pensamento em mente, chegamos que quando $t=2\pi$ a representação da circunferência está completa. Dessa forma, o \ttt{ParametricPlot} alcança a seguinte forma após assumirmos que $x$ e $y$ dependem de $t$
...
Para representarmos $p_{1}$ e $p_{2}$ graficamente vamos usar o comando \ttt{Graphics3D}. Como vamos plotar pontos usamos o comando \ttt{Point} dentro dele da seguinte forma 
...
Vamos fazer um paraboloide análogo ao que foi feito com o \ttt{ContourPlot}. Basta estendermos a ideia do \ttt{ParametricPlot} para duas dimensões alterando a terceira posição da função ($f_{z}$). 
...
Podemos interpretar a linha acima como: ``A partir da variação de \ttt{t}, renderize varias circunferências com o raio e altura variando segundo \ttt{r} (de 0 a 4 para o raio e de 0 a 16 para a altura)''
\end{document}
